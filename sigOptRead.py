...
for k in RS['iParamRanges'].keys():
    ...
    plt.savefig(resolvePath(["figures",name,
                             "diff2v1Scater"+str(k)+".png"],
                            relative=True))
...
for k in RS['iParamRanges'].keys():
    ...
    plt.savefig(resolvePath(["figures",name,
                             "diff3v1Scater"+str(k)+".png"],
                            relative=True))
badCandidates.append(int(temp.iloc[0]["row"]))
medianCandidates.append(int(temp.iloc[len(temp)//2]["row"]))
goodCandidates.append(int(temp.iloc[-1]["row"]))

def myPlot(candidates, saveType=None):
    for row in candidates:
        ...
        if saveType is not None:
            plt.savefig(resolvePath(["figures",name,
                                     saveType+"-"+str(row)+".png"],
                                    relative=True))
        
myPlot(badCandidates,saveType="badCandidates")
myPlot(medianCandidates,saveType="medianCandidates")
myPlot(goodCandidates,saveType="goodCandidates")
